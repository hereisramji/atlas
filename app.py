...
# Section 2: Looking at Data
    ...
    cohort_data = {
        "Cohort ID": [1, 2, 3, 4, 5, 6],
        "Indication": ["Melanoma", "UC and Bladder", "Melanoma", "Melanoma", "NSCLC", "Melanoma"],
        "Number of Specimens": [76, 204, 468, 386, 35, 501],
        "Number of Patients": [34, 79, 257, 142, 12, 132],
        "Analyzed Specimens": [76, 204, 460, 239, 35, 501]
    }
    ...
    for i, indication in enumerate(cohort_data["Indication"]):
        if indication not in indication_to_cohorts:
            indication_to_cohorts[indication] = []
        indication_to_cohorts[indication].append(cohort_data["Cohort ID"][i])
    ...
    cohort_labels = [f"Cohort {cid}: {selected_indication} ({cohort_data['Number of Patients'][cohort_data['Cohort ID'].index(cid)]} patients, {cohort_data['Number of Specimens'][cohort_data['Cohort ID'].index(cid)]} specimens)" for cid in cohort_options]
    ...
    cohort_idx = cohort_data["Cohort ID"].index(selected_cohort_id)
    st.write(f"""
    **Selected: Cohort {selected_cohort_id}**
    - Indication: {cohort_data['Indication'][cohort_idx]}
    - Patients: {cohort_data['Number of Patients'][cohort_idx]}
    - Total Specimens: {cohort_data['Number of Specimens'][cohort_idx]}
    - Analyzed Specimens: {cohort_data['Analyzed Specimens'][cohort_idx]}
    """)

    # Interactive Marker Expression Analysis Tool - now linked to selected cohort
    ...
    Select your target marker to see expression patterns that may predict response in {cohort_data['Indication'][cohort_idx]} patients.
    """)
    ...
    for disease in marker_data:
        for marker in other_markers:
            # Generate some random but plausible data - would be replaced with real data
    ...
    # Get markers for the selected indication
    selected_indication = cohort_data['Indication'][cohort_idx]
    
    if selected_indication in marker_data:
        markers = list(marker_data[selected_indication].keys())
        selected_marker = st.selectbox("Select Target Marker", markers)
        
        # Create a DataFrame for the selected data
        selected_data = marker_data[selected_indication][selected_marker]
        df = pd.DataFrame({
        ...
        st.write(f"### {selected_marker} Expression in {selected_indication} by Response Status (Cohort {selected_cohort_id})")
        ...
        st.write(f"""
        Based on Teiko's analysis of Cohort {selected_cohort_id}, {selected_marker} expression shows **{predictive_value} predictive value** for treatment response in {selected_indication} patients:
        ...
        if responder_trend > 0 and nonresponder_trend < 0:
            st.write(f"""
            - Consider {selected_marker} as a potential pharmacodynamic biomarker for your drug in {selected_indication}
            ...
            st.write(f"""
            - {selected_marker} shows subtle differences between responders and non-responders in {selected_indication}
        ...
        if show_advanced:
            ...
            with tab1:
                ...
                st.write(f"This chart shows the probability of response based on {selected_marker} expression levels at different timepoints.")
            ...
            with tab2:
                ...
                st.write(f"How {selected_marker} expression correlates with other markers in {selected_indication} (Cohort {selected_cohort_id}).")
        ...
        st.warning(f"No marker data available for {selected_indication}. Please select another indication.")
